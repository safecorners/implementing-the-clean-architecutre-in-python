...
@pytest.fixture()
def process_manager(
    payments_facade_mock: Mock, customer_relationship_mock: Mock
) -> PayingForWonItem:
...
def test_should_send_success_email_after_payment(
    ...
    customer_relationship_mock.send_email_after_successful_payment.assert_called_once_with(
        event.customer_id,
        pm_data_waiting_for_payment.winning_bid,
        pm_data_waiting_for_payment.auction_title,
    )
