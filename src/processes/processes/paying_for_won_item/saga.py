...
class PayingForWonItem:
    ...
    def handle_auction_ended(
        self, event: AuctionEnded, data: PayingForWonItemData
    ) -> None:
        assert data.state is None
        payment_uuid = uuid.uuid4()
        self._payments.start_new_payment(
            payment_uuid, event.winner_id, event.winning_bid, event.auction_title
        )
    ...
    def handle_payment_captured(
        self, event: PaymentCaptured, data: PayingForWonItemData
    ) -> None:
        assert data.state == State.PAYMENT_STARTED
