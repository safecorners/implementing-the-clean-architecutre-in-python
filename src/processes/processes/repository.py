...
class ProcessManagerDataRepo:
    ...
    def get(self, process_uuid: UUID, data_cls: Type[T]) -> T:
        row = self._connection.execute(
            process_manager_data_table.select().where(
    ...
    def save(self, process_uuid: UUID, data: T) -> None:
        ...
        if row:
            self._connection.execute(
                process_manager_data_table.update()
                .values({"json": data})
                .where(process_manager_data_table.c.uuid == process_uuid)
            )
        else:
            self._connection.execute(
                process_manager_data_table.insert().values(
                    {"uuid": process_uuid, "json": data}
                )
            )
