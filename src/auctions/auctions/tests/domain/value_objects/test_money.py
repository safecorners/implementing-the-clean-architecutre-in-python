...
from auctions.domain.value_objects.money import Currency, Money
from auctions.domain.value_objects.money.currency import USD
...
def teset_invalid_inputs(currency: object, amount: object) -> None:
...
@pytest.mark.parametrize(
    "money_instance, expected_str",
    [(Money(USD, "12.49"), "12.49 $"), (Money(BTC, "0.00004212"), "0.00004212 Ƀ")],
)
...
def test_supports_different_comparison_operators(
    one: Money,
    another: Money,
    cmp_operator: Callable[[Money, Money], bool],
...
def test_supports_basic_math_operators_if_the_same_currency(
    one: Money,
    another: Money,
    math_operator: Callable[[Money, Money], Money],
    expected_result: Money,
) -> None:
...
@pytest.mark.parametrize(
    "arg, expected_result",
    [("5.0000", Money(USD, "5")), (Decimal("5.990000000"), Money(USD, "5.99"))],
)
