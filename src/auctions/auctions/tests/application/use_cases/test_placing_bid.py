...
def test_makes_an_expected_bid(
    placing_bid_uc: PlacingBid, input_dto: PlacingBidInputDto, auction: Auction
) -> None:
...
def test_presents_output_dto(
    ...
    expected_output_dto = PlacingBidOutputDto(
        is_winner=True, current_price=input_dto.amount
    )
    placing_bid_output_boundary_mock.present.assert_called_once_with(
        expected_output_dto
    )
