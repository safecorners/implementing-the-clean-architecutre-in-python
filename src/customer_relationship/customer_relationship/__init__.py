...
class CustomerRelationship(injector.Module):
    ...
    def configure(self, binder: injector.Binder) -> None:
        binder.multibind(
            ...
            to=AsyncEventHandlerProvider(BidderHasBeenOverbidHandler),
        )
        binder.multibind(
            AsyncHandler[WinningBidPlaced],
            to=AsyncEventHandlerProvider(WinningBidPlacedHandler),
        )
...
class WinningBidPlacedHandler:
    ...
    def __call__(self, event: WinningBidPlaced) -> None:
        self._facade.send_email_about_winning(
            event.bidder_id, event.bid_amount, event.auction_title
        )
