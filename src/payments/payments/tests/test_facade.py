...
def inserted_payment(request: SubRequest, connection: Connection) -> dict:
    ...
    charge_id = (
        None
        if status not in (PaymentStatus.CHARGED.value, PaymentStatus.CAPTURED.value)
        else "token"
    )
    data: Dict[str, Any] = {
        "uuid": str(uuid.uuid4()),
    ...
    connection.execute(payments.insert().values(data))
    return data


def get_payment(connection: Connection, payment_uuid: str) -> Row:
    row = connection.execute(
        payments.select().where(payments.c.uuid == payment_uuid)
    ).first()
    return row
...
def test_successful_charge_updates_status(
    ...
    charge_mock.assert_called_once_with(
        get_usd(inserted_payment["amount"] / 100), "token"
    )
